...
class PieView < Qt::AbstractItemView
	...
	def indexAt(point)
	    ...
	    if wx < @totalSize
	        ...
	        (0...rowCount(rootIndex())).each do |row|
	...
	def setSelection(rect, command)
	    ...
	    indexes = Qt::ModelIndexList.new
	
		(0...rows).each do |row|
			(0...columns).each do |column|
	            index = model().index(row, column, rootIndex())
	            region = itemRegion(index)
	            if !region.intersect(contentsRect).empty?
	...
	def visualRegionForSelection(selection)
	    ...
	    firstItem = model().index(qMin(firstRow, lastRow), 0, rootIndex())
	    lastItem = model().index(qMax(firstRow, lastRow), 0, rootIndex())
